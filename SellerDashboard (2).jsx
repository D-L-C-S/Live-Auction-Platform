...
import { createAuction, getSellerListings, fetchEscrow } from "./api";

const API_BASE = "http://localhost:5000"; // update to deployed URL — kept for reference

const ENDPOINTS = {
  createListing:  `${API_BASE}/api/auctions`,
  sellerListings: `${API_BASE}/api/auctions?seller=me`,
  getAuction: (id) => `${API_BASE}/api/auctions/${id}`,
};

// ─── Route Guard ──────────────────────────────────────────────────────────────
// Redirect to /login immediately if no auth token is present in localStorage.
...
// ─── Escrow Badge ─────────────────────────────────────────────────────────────
// Safely handles null / undefined / unexpected values — never renders raw "null".
const ESCROW_STYLES = {
...
// ─── Map raw API auction object → table row shape ─────────────────────────────
function toRow(a) {
  return {
    id:            a._id,
    title:         a.title,
    ...
    endsIn:        timeUntil(a.auctionEndTime),
    status:        a.status ?? "active",
    // escrow.status may be null/missing on active listings — normalise here
    escrowStatus:  (a.escrow?.status && a.escrow.status !== "null") ? a.escrow.status : null,
  };
}
...
export default function SellerDashboard() {
  ...
  // ── Fetch seller's listings on mount, then hydrate escrow status ────────────
  useEffect(() => {
    (async () => {
      try {
        const data = await getSellerListings();
        const raw  = Array.isArray(data) ? data : (data.auctions ?? []);
        const rows = raw.map(toRow);
        setListings(rows);

        // For every closed/settled listing, fetch live escrow status
